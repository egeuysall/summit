import type { Blog } from '@/types/other';

export const blogs: Blog[] = [
	{
		slug: 'astroexplorers-have-traveled-to-the-north-of-the-moon',
		title: 'AstroExplorers have traveled to the north of the moon.',
		excerpt:
			'Astrounautes say that "North of the current colony settlemrnt, they can expand more across the moon."',
		content: `### AstroExplorers Journey North

Astronauts ventured north of the main colony settlement, their boots pressing into the powdery lunar soil. As they explored the uncharted terrain, they discovered a promising plateau bathed in sunlight.

Working together, the team began setting up a new outpost, laying the foundation for another thriving lunar colony. Their efforts marked a bold step in expanding the human races presence on the moon.`,
	},
	{
		slug: 'getting-started-with-solaris-farming-on-the-moon-tech',
		title: 'Getting Started with Solaris farming on the moon tech.',
		excerpt: 'Colonists on the moon have started using Solaris tech to farm food and grow plants.',
		content: `### Solaris Farming Technology

The colonists on the moon quickly adapted to their new environment, determined to make life sustainable beyond Earth. Using advanced **Solaris technology**, they began farming in specially designed lunar greenhouses.

Harnessing abundant solar energy, they cultivated crops in nutrient-rich substrates, turning the barren lunar surface into a patchwork of thriving gardens. This breakthrough marked a new era of self-sufficiency for the moon's settlers.`,
	},
	{
		slug: 'scientists-on-the-moon-have-developed-new-water-harvesting-techniques',
		title: 'Scientists on the moon have developed new water harvesting techniques.',
		excerpt:
			'Colonists on the moon have started using new techniques to harvest water from the lunar surface.',
		content: `### Water Harvesting Breakthrough

To support their growing community, colonists on the moon have developed innovative techniques to harvest water from the lunar surface.

By extracting and purifying ice found in shadowed craters, they are able to provide clean water for:
- Drinking
- Farming
- Other essential needs

This breakthrough has greatly improved the sustainability of life on the moon, paving the way for further expansion and exploration.`,
	},
];
